...
```
```{r}
x <- select(filter(gapminder, continent =="Asia", year >= 1970, year <= 1979), country, year, gdpPercap)
library(knitr)
kable(x, digits = 5, row.names = (gapminder$country), col.names = (gapminder$year), c, caption = "GDP per captia of Asian countries in 70s",format.args = escape = TRUE)
```
