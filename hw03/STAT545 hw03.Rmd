...
```
In a larger scale, there is not much difference among the minimum GDP per capita of the continents. However, the maximum GDP per capita of Asia is much more higher than those of other continents. 
...
l3 <- l1 %>%
  group_by(continent) %>%
  summarize_each(lifeExp_change = lifeExp)
```
...
ll1 <- gapminder %>%
  ...
  group_by(year) %>%
  summarize_each(n_countries = length())
...
# Report my process
1. I struggled on counting the occurence of a certain variable within a group.
