...
numcountry_lowlife<- gapminder %>%
  ...
  group_by(continent) %>%
  ggplot(aes(x=year, y=distinct(country))) +
  geom_bar(aes(x=year, y=distinct(country), fill=continent),position="dodge") +
  scale_fill_manual(values=brewer.pal(n=5, "Set2")) +
...
```{r}
country <- c("Belgium", "Canada", "China", "Finland")
national_animal <- c("Lion", "Beaver", "Panda", "Brown bear")
```
